// https://csrc.nist.gov/csrc/media/publications/fips/197/final/documents/fips-197.pdf

// probably the most unoptimized implementation of aes you've ever seen, but it was a great way to understand (a bit) of aes's internals

use symmetric::aes_dfa::AesFault;
use std::cmp::Ordering;

// Nk : Number of 32 bits words in the cipher key (4, 6 or 8)
// Nr : Number of rounds. (10, 12 or 14)
// Nb : Number of columns. 4 in the standard

const NB: u8 = 4;

pub const S_BOX: [u8; 256] = [
    ,
];

pub const INV_S_BOX: [u8; 256] = [
    ,
];

pub fn rcon(mut i: u8) -> Vec<u8> {
    let mut r: Vec<u8> = vec![0x02, 0x00, 0x00, 0x00];

    match i.cmp(&1) {
        Ordering::Equal => r[0] = 0x01,
        Ordering::Greater => {
            r[0] = 0x02;
            i -= 1;
            while i - 1 > 0 {
                r[0] = gf256_mul(r[0], 0x02);
                i -= 1;
            }
        },
        Ordering::Less => (),
    }

    r
}

// Addition in GF is xor
pub fn gf256_add(a: u8, b: u8) -> u8 {
    a ^ b
}

// Same for substraction
pub fn gf256_sub(a: u8, b: u8) -> u8 {
    a ^ b
}

pub const RCON: [u8; 256] = [
    ,
];

pub fn gf256_mul(mut a: u8, mut b: u8) -> u8 {

    // Multplication in GF256
    
    let mut p: u8 = 0;
    let mut carry: u8;

    for _ in 0..8 {
        if a == 0 || b == 0 {
            break;
        }

        if b & 1 == 1 {
            p ^= a;
        }

        b >>= 1;
        carry = a & 128;

        a <<= 1;
        if carry == 128 {
            a ^= 0x1b;
        }
    }
    p
}

pub fn gf256_poly_add(a: [u8; 4], b: [u8; 4]) -> [u8; 4] {
    // addition term by term in GF256[256]
    let mut c = [0u8; 4];
    c[0] = a[0] ^ b[0];
    c[1] = a[1] ^ b[1];
    c[2] = a[2] ^ b[2];
    c[3] = a[3] ^ b[3];
    c
}

pub fn gf256_poly_mul(a: [u8; 4], b: [u8; 4]) -> [u8; 4] {

    // Multplication in GF256[X]

    let mut d = [0u8; 4];
    d[0] = gf256_mul(a[0], b[0]) ^ gf256_mul(a[3], b[1]) ^ gf256_mul(a[2], b[2])
        ^ gf256_mul(a[1], b[3]);

    d[1] = gf256_mul(a[1], b[0]) ^ gf256_mul(a[0], b[1]) ^ gf256_mul(a[3], b[2])
        ^ gf256_mul(a[2], b[3]);

    d[2] = gf256_mul(a[2], b[0]) ^ gf256_mul(a[1], b[1]) ^ gf256_mul(a[0], b[2])
        ^ gf256_mul(a[3], b[3]);

    d[3] = gf256_mul(a[3], b[0]) ^ gf256_mul(a[2], b[1]) ^ gf256_mul(a[1], b[2])
        ^ gf256_mul(a[0], b[3]);
    d
}


// Place the first index at the end (aka cyclic permutation)
pub fn rot_word(w: &mut [u8; 4]) {

    let t = w[0];
    w[0] = w[1];
    w[1] = w[2];
    w[2] = w[3];
    w[3] = t;
}

// Substitute every byte of the word `w`
pub fn sub_word(w: &mut [u8; 4]) {
    for i in 0..4 {
        w[i] = S_BOX[w[i] as usize];
    }

}

pub fn sub_bytes(state: &mut [u8; 16]) {

    for i in 0..16 {
        state[i] = S_BOX[state[i] as usize];
    }

}

pub fn inv_sub_bytes(state: &mut [u8; 16]) {
    for i in 0..16 {
        state[i] = INV_S_BOX[state[i] as usize];
    }
}

pub fn add_round_key(key: &[u8], state: &mut [u8; 16], r: u8) {
    for i in 0..16 {
        state[i] ^= key[(4 * NB * r) as usize + i];
    }
}

pub fn inv_shift_rows(state: &mut [u8; 16]) {

    let mut temp = vec![0; 16];

    temp[0] = state[0];
    temp[1] = state[13];
    temp[2] = state[10];
    temp[3] = state[7];

    temp[4] = state[4];
    temp[5] = state[1];
    temp[6] = state[14];
    temp[7] = state[11];

    temp[8] = state[8];
    temp[9] = state[5];
    temp[10] = state[2];
    temp[11] = state[15];

    temp[12] = state[12];
    temp[13] = state[9];
    temp[14] = state[6];
    temp[15] = state[3];

    state[..16].clone_from_slice(&temp[..16]) 

}

pub fn shift_rows(state: &mut [u8; 16]) {
    let mut temp = vec![0; 16];
    temp[0] = state[0];
    temp[1] = state[5];
    temp[2] = state[10];
    temp[3] = state[15];

    temp[4] = state[4];
    temp[5] = state[9];
    temp[6] = state[14];
    temp[7] = state[3];

    temp[8] = state[8];
    temp[9] = state[13];
    temp[10] = state[2];
    temp[11] = state[7];

    temp[12] = state[12];
    temp[13] = state[1];
    temp[14] = state[6];
    temp[15] = state[11];

    state[..16].clone_from_slice(&temp[..16])
}

#[allow(clippy::identity_op)]
pub fn mix_columns(state: &mut [u8; 16]) {
    for i in 0..4 {
        let b0 = state[4 * i + 0];
        let b1 = state[4 * i + 1];
        let b2 = state[4 * i + 2];
        let b3 = state[4 * i + 3];

        state[4 * i + 0] = gf256_mul(2, b0) ^ gf256_mul(3, b1) ^ b2 ^ b3;
        state[4 * i + 1] = b0 ^ gf256_mul(2, b1) ^ gf256_mul(3, b2) ^ b3;
        state[4 * i + 2] = b0 ^ b1 ^ gf256_mul(2, b2) ^ gf256_mul(3, b3);
        state[4 * i + 3] = gf256_mul(3, b0) ^ b1 ^ b2 ^ gf256_mul(2, b3);
    }
}

#[allow(clippy::identity_op)]
pub fn inv_mix_columns(state: &mut [u8; 16]) {
    for i in 0..4 {
        let b0 = state[4 * i + 0];
        let b1 = state[4 * i + 1];
        let b2 = state[4 * i + 2];
        let b3 = state[4 * i + 3];

        state[4 * i + 0] = gf256_mul(14, b0) ^ gf256_mul(11, b1) ^ gf256_mul(13, b2) ^ gf256_mul(9 , b3);
        state[4 * i + 1] = gf256_mul(9 , b0) ^ gf256_mul(14, b1) ^ gf256_mul(11, b2) ^ gf256_mul(13, b3);
        state[4 * i + 2] = gf256_mul(13, b0) ^ gf256_mul(9 , b1) ^ gf256_mul(14, b2) ^ gf256_mul(11, b3);
        state[4 * i + 3] = gf256_mul(11, b0) ^ gf256_mul(13, b1) ^ gf256_mul(9 , b2) ^ gf256_mul(14, b3);
    }
}

//https://en.wikipedia.org/wiki/Rijndael_key_schedule
//http://www.samiam.org/key-schedule.html
#[allow(clippy::identity_op)]
pub fn key_expansion(k: &[u8], nk: u8, nr: u8) -> Vec<u8> {
    let mut tmp: [u8; 4] = [0, 0, 0, 0];

    let len = NB * (nr + 1);
    let mut w: Vec<u8> = Vec::new();

    //init the vector
    for _ in 0..len * 4 {
        w.push(0);
    }

    for i in 0..nk {
        w[(4 * i + 0) as usize] = k[(4 * i + 0) as usize];
        w[(4 * i + 1) as usize] = k[(4 * i + 1) as usize];
        w[(4 * i + 2) as usize] = k[(4 * i + 2) as usize];
        w[(4 * i + 3) as usize] = k[(4 * i + 3) as usize];
    }

    for i in nk..len {
        tmp[0] = w[(4 * (i - 1) + 0) as usize];
        tmp[1] = w[(4 * (i - 1) + 1) as usize];
        tmp[2] = w[(4 * (i - 1) + 2) as usize];
        tmp[3] = w[(4 * (i - 1) + 3) as usize];

        if i % nk == 0 {
            rot_word(&mut tmp);
            sub_word(&mut tmp);
            let tr = [RCON[(i / nk) as usize], 0, 0, 0];
            tmp = gf256_poly_add(tmp, tr);

        } else if nk > 6 && i % nk == 4 {
            sub_word(&mut tmp);
        }

        w[(4 * i + 0) as usize] = w[(4 * (i - nk) + 0) as usize] ^ tmp[0];
        w[(4 * i + 1) as usize] = w[(4 * (i - nk) + 1) as usize] ^ tmp[1];
        w[(4 * i + 2) as usize] = w[(4 * (i - nk) + 2) as usize] ^ tmp[2];
        w[(4 * i + 3) as usize] = w[(4 * (i - nk) + 3) as usize] ^ tmp[3];
    }

    w
}

pub fn round(key: &[u8], state: &mut [u8; 16], r: u8, fault: AesFault) {
    sub_bytes(state);
    shift_rows(state);

	if r == fault.round {
		state[fault.index as usize] ^= fault.value;
	}

    mix_columns(state);
    add_round_key(key, state, r);
}

pub fn final_round(key: &[u8], state: &mut [u8; 16], nr: u8) {
    sub_bytes(state);
    shift_rows(state);
    add_round_key(key, state, nr);
}

pub fn inv_round(key: &[u8], state: &mut [u8; 16], r: u8) {
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(key, state, r);
    inv_mix_columns(state);
}

pub fn inv_final_round(key: &[u8], state: &mut [u8; 16]) {
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(key, state, 0);
}

pub fn cipher(key: &[u8], state: &mut [u8; 16], nr: u8, fault:AesFault) {
    add_round_key(key, state, 0);

    for r in 1..nr {
        round(key, state, r, fault);
    }

   final_round(key, state, nr);
}

pub fn inv_cipher(key: &[u8], state: &mut [u8; 16], nr: u8) {

    add_round_key(key, state, nr);
    for r in (1..nr).rev() {
        inv_round(key, state, r);
    }
    inv_final_round(key, state);

}
